<?php
    ...
    $estaEmpresa=(new Empresas)->read($id);
    $subiImagen=false;
    ...
    if(isset($_POST['enviar'])){
        ...
        if(is_uploaded_file($_FILES['logo']['tmp_name'])){
            if((new Imagen)->isImagen($_FILES['logo']['type'])){
                ...
                if($imagen->guardarImagen($_FILES['logo']['tmp_name'])){
                    ...
                    if(basename($estaEmpresa->logo) !='default.png'){
                        $imagen->borrarFichero(basename($estEmpresa->logo));
                    }
                    $subiImagen=true;
                }else{
                    $error=true;
        ...
        if(!$error){
            if(!$subiImagen){
                $empresa->setNombre($n)->setFacturacion($f)->setLogo($estaEmpresa->logo);
            }else{
                $empresa->setNombre($n)->setFacturacion($f);
            }
            $empresa->update($id);
...
<body style="background-color:#1ABC9C">
    <h4 class="text-center">Actualizar Empresa</h4>
        ...
        <div class="my-2 p-4 mx-auto bg-secondary text-white" style=" width:40rem">
            <form name="s" action='<?php echo $_SERVER['PHP_SELF']."?id=$id"; ?>' method='POST' enctype="multipart/form-data">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="n" value="<?php echo $estaEmpresa->nombre; ?>" name="nombre">
                    <?php
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="f" value="<?php echo $estaEmpresa->facturacion; ?>" name="facturacion">
                    <?php
                ...
                <div class="mb-3">
                    <button type="submit" name="enviar" class="btn btn-success"><i class="fas fa-save"></i> Actualizar</button>
